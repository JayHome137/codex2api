...
func (db *DB) migrateSQLite(ctx context.Context) error {
	...
	columns := []struct {
		...
		{"usage_logs", "upstream_cost", "REAL DEFAULT 0"},
		{"usage_logs", "upstream_rate_multiplier", "REAL DEFAULT 0"},
		{"usage_logs", "upstream_cost_available", "INTEGER DEFAULT 0"},
		{"usage_logs", "upstream_cost_provider", "TEXT DEFAULT ''"},
